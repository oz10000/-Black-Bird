# exchange.py
# ============================================================
# CLIENTE OKX CON SINCRONIZACIÓN HORARIA
# ============================================================
...
class Exchange:
    ...
    def place_algo_order(self, symbol, side, trigger_price, order_price, size, order_type="conditional"):
        if not self._connected:
            return {"ok": False, "error": "No conectado"}
    ...
    def place_trailing_order(self, symbol, side, size, callback_rate):
        if not self._connected:
            return {"ok": False, "error": "No conectado"}
        path = "/api/v5/trade/order-algo"
        body = {
    ...
    def get_pending_algo_orders(self, symbol=None):
        ...
        params = {"ordType": "conditional,oco,trigger,move_order_stop"}
        if symbol:
            params["instId"] = symbol
        headers = self._sign_request("GET", path, params=params)
        resp = self._retry(self.session.get, f"{self.base_url}{path}", headers=headers, params=params, timeout=ORDER_TIMEOUT)
